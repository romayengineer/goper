...
func buildSNIExtension(sni string) []byte {
	...
	data.Write([]byte{0x00, byte(3 + len(name))}) // server name list length
	data.WriteByte(0x00)                          // name type: host_name
	data.Write([]byte{byte(len(name) >> 8), byte(len(name))})
	data.Write(name)

	var ext bytes.Buffer
	ext.Write([]byte{0x00, 0x00}) // extension type: server_name (0)
	ext.Write([]byte{0x00, byte(data.Len())})
...
func buildClientHelloRecord(sni string) []byte {
	...
	extensions.Write([]byte{byte(len(ext) >> 8), byte(len(ext))})
	...
	hl := handshake.Len()
	record.Write([]byte{byte(hl >> 8), byte(hl)})
