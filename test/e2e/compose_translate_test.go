...
// Package e2e — translate offer suppression test.
//
// Brings up the compose stack (Xvfb overlay for headless/CI), drives Chrome to
// a Spanish page, and asserts that Chromium does NOT offer to translate it.
// The offer is browser chrome (not reachable via page DOM/AX), so the probe
// reads chrome://translate-internals: a non-zero ignored/denied count for "es"
// means an offer was shown.
package e2e
...
func TestTranslateOfferSuppressed(t *testing.T) {
	requireDocker(t)
	root := repoRoot(t)
	...
	// 4. Probe: visit a Spanish page and report whether a translate offer fired.
	ctx, cancel := context.WithTimeout(context.Background(), 90*time.Second)
	out, err = composeExec(ctx, root, "chrome", "node", "/app/translate-probe.js")
	cancel()
	require.NoError(t, err, "translate probe failed:\n%s", out)
	assert.Contains(t, out, "OFFER:absent", "chromium must not offer to translate a Spanish page")
}
